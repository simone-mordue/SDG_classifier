...
d='/Users/simonemordue/OneDrive - Newcastle University/SDG_Classifier/SDGclassy-master' 	#specify root path
...
#Import target data and create inferencer
mallet/bin/mallet import-dir --input "${d}"/"${p}"/"${f}"  --output "${d}"/"${p}"/workshop/inferring-"${f}".mallet --keep-sequence --remove-stopwords --extra-stopwords "${d}"/"${p}"/workshop/extra-exclude-words_new.txt --keep-sequence-bigrams --gram-sizes 1,2  --use-pipe-from "${d}"/classifier/results/"${c}".mallet
